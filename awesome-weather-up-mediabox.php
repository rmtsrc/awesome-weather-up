...
if (is_admin()) {
	...
	function awesome_weather_media_button()
	{
		...
		if (version_compare($wp_version, '3.5', '>=') and in_array($pagenow, array('post.php', 'page.php', 'post-new.php', 'post-edit.php'))) {
			$img = '<style>#awe-thick-location.error { border: solid 1px red; } #awesome-weather-up-media-button::before { font: 400 18px/1 dashicons; content: \'\f176\'; }</style><span class="wp-media-buttons-icon" id="awesome-weather-up-media-button"></span>';
			$output = '<a href="#TB_inline&width=750&height=500&inlineId=add-awesome-weather-up" class="thickbox button awesome-weather-up-thickbox" title="' .  __('Add Awesome Weather Widget Updated Shortcode', 'awesome-weather-up') . '" style="padding-left: .4em;"> ' . $img . __('Add Weather', 'awesome-weather-up') . '</a>';
		}
		echo $output;
	}
	...
	function awesome_weather_admin_footer_for_thickbox()
	{
		...
		if (version_compare($wp_version, '3.5', '>=') and in_array($pagenow, array('post.php', 'page.php', 'post-new.php', 'post-edit.php'))) { ?>
			<script type="text/javascript">
				function insert_awesome_weather() {
					...
					if (awe_location) {
						...
						window.send_to_editor("[awesome-weather-up" + add_to_shortcode + "]");
					} else {
			...
			<div id="add-awesome-weather-up" style="display: none;">
				<div class="wrap" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif;">

					<div>
						<?php _e('Location:', 'awesome-weather-up'); ?>
						<input type="text" id="awe-thick-location" name="awe-location" value="" placeholder="<?php _e('Required', 'awesome-weather-up'); ?>">
					</div>

					<div style="padding: 10px 0px;">
						<?php _e('Template:', 'awesome-weather-up'); ?> &nbsp;
					...
					<div id="awe-thick-custom-template-wrap" style="padding-bottom: 10px; display: none;">
						<label for="awe-thick-custom-template"><?php _e('Custom Template Filename:', 'awesome-weather-up'); ?></label> <small>(<?php _e('found in theme folder', 'awesome-weather-up'); ?>)</small><br>
						<?php echo $theme_folder; ?>/awe-&nbsp;<input id="awe-thick-custom-template" type="text" value="" style="width: 80px; font-size: 11px;" />&nbsp;.php
					</div>

					<div style="padding-bottom: 10px;">
						<?php _e('Units:', 'awesome-weather-up'); ?>&nbsp;
					...
					<div style="padding: 10px 0px;">
						<input type="button" class="button-primary" value="<?php echo __('Insert Weather Widget', 'awesome-weather-up'); ?>" onclick="insert_awesome_weather();" />
					</div>

					<hr>

					<h3><?php _e('Optional', 'awesome-weather-up'); ?></h3>

					<div style="padding-bottom: 20px;">
						<?php _e('OpenWeatherMap City ID:', 'awesome-weather-up'); ?>
						<input type="number" id="awe-owm-city-id" value="">
					</div>

					<div style="padding-bottom: 10px;">
						<?php _e('Forecast:', 'awesome-weather-up'); ?>
						<select id="awe-thick-forecast_days">
							<option value=""><?php _e('Default', 'awesome-weather-up'); ?></option>
							<?php for ($d = 1; $d <= 15; $d++) { ?>
								<option value="<?php echo $d; ?>"> <?php echo sprintf(_n('%s Day', '%s Days', $d, 'awesome-weather-up'), $d); ?></option>
							<?php } ?>
							<option value="hide"><?php _e("Don't Show", 'awesome-weather-up'); ?></option>
						</select>
					</div>

					<div style="padding-bottom: 10px;">
						<?php _e('Banner Title:', 'awesome-weather-up'); ?>
						<input type="text" id="awe-thick-override_title" value="">
					</div>

					<div style=" padding-bottom: 10px; display: inline-block">
						<?php _e('Widget Title:', 'awesome-weather-up'); ?>
					...
					<div style="margin: 10px 0; padding: 20px; background: #efefef;">
						<div style="padding-bottom: 7px;">
							<?php _e('Background Image:', 'awesome-weather-up'); ?>
						...
						<div style="padding: 10px 0;">
							...
							<label for="awe-thick-background_by_weather"><?php _e('Use Different Background Images Based on Weather', 'awesome-weather-up'); ?></label> <a href="https://halgatewood.com/docs/plugins/awesome-weather-up-widget/creating-different-backgrounds-for-different-weather" target="_blank">(?)</a> &nbsp;
						</div>
						<div>
							<?php _e('Custom Background Color:', 'awesome-weather-up'); ?><br>
							<input type="text" id="awe-thick-custom_bg_color" value="" style="width: 60%;" placeholder="#7fb761 / rgba(0,0,0,0.5)">
						</div>
					</div>

					<div>
						<?php _e('Text Color', 'awesome-weather-up'); ?><br>
					...
					<div style="padding-bottom: 10px;">
						...
						<label for="awe-thick-use_user_location"><?php _e('Use User Location', 'awesome-weather-up'); ?></label>
					</div>

					<div style="padding-bottom: 10px;">
						<input type="checkbox" id="awe-thick-allow_user_to_change" value="">
						<label for="awe-thick-allow_user_to_change"><?php _e('Allow User to Change the Location', 'awesome-weather-up'); ?></label>
					</div>

					<div style="padding-bottom: 10px;">
						<input type="checkbox" id="awe-thick-show_icons" value="">
						<label for="awe-thick-show_icons"><?php _e('Show Weather Icons', 'awesome-weather-up'); ?></label>
					</div>

					<div style="padding-bottom: 10px;">
						<input type="checkbox" id="awe-thick-hide_stats" value="">
						<label for="awe-thick-hide_stats"><?php _e('Hide Stats', 'awesome-weather-up'); ?></label>
					</div>

					<div style="padding-bottom: 10px;">
						<input type="checkbox" id="awe-thick-hide_attribution" value="">
						<label for="awe-thick-hide_attribution"><?php _e('Hide Weather Attribution', 'awesome-weather-up'); ?></label>
					</div>

					<div style="padding-bottom: 10px;">
						<input id="awe-thick-show-link" type="checkbox" value="1" />
						<label for="awe-thick-show-link"><?php _e('Link to Extended Forecast', 'awesome-weather-up'); ?></label>
					</div>

					<div style="padding-bottom: 10px;">
						<label for="awe-thick-extended-url"><?php _e('Custom Extended Forecast URL:', 'awesome-weather-up'); ?></label><br>
						<input id="awe-thick-extended-url" type="text" value="" style="width: 60%;">
					</div>

					<div style="padding-bottom: 10px;">
						<label for="awe-thick-extended-text"><?php _e('Custom Extended Forecast Text:', 'awesome-weather-up'); ?></label><br>
						<input id="awe-thick-extended-text" type="text" value="" style="width: 60%;">
					</div>

					<div style="padding: 10px 0px;">
						<input type="button" class="button-primary" value="<?php echo __('Insert Weather Widget', 'awesome-weather-up'); ?>" onclick="insert_awesome_weather();" />
						<a class="button-secondary" onclick="tb_remove();" title="<?php _e('Cancel', 'awesome-weather-up'); ?>"><?php _e('Cancel', 'awesome-weather-up'); ?></a>
					</div>
				</div>
			</div>
<?php
