...
function awesome_weather_get_weather_data_openweathermaps(&$weather)
{
	...
	// WIND LABELS
	$awe_wind_label = array(__('N', 'awesome-weather-up'), __('NNE', 'awesome-weather-up'), __('NE', 'awesome-weather-up'), __('ENE', 'awesome-weather-up'), __('E', 'awesome-weather-up'), __('ESE', 'awesome-weather-up'), __('SE', 'awesome-weather-up'), __('SSE', 'awesome-weather-up'), __('S', 'awesome-weather-up'), __('SSW', 'awesome-weather-up'), __('SW', 'awesome-weather-up'), __('WSW', 'awesome-weather-up'), __('W', 'awesome-weather-up'), __('WNW', 'awesome-weather-up'), __('NW', 'awesome-weather-up'), __('NNW', 'awesome-weather-up'));
	...
	if (isset($weather->latlng) and $weather->latlng) {
		...
		if (!$latlng->lat and !$latlng->lng) {
			return array('error' => true, 'msg' => __('Weather Geolocation Not Found', 'awesome-weather-up'));
		}
		...
		if ($city_id_from_location) {
			...
			return array('error' => true, 'msg' => __('Weather Location Not Found', 'awesome-weather-up') . ': <a href="https://openweathermap.org/find?q=' . $weather->location . '" target="_blank" style="color: #fff;">' . $weather->location . '</a>');
		}
	}
	...
	if ($weather->owm_city_id and is_numeric($weather->owm_city_id)) {
		...
		return array('error' => true, 'msg' => __('Weather Location Not Set', 'awesome-weather-up'));
	}
	...
	if (isset($city_data->cod) and $city_data->cod == 404) {
		...
		if (isset($city_data->wind) and $city_data->wind) {
			...
			$wind_speed_text 	= __('m/s', 'awesome-weather-up');
...
function awesome_weather_build_hourly_forecast(&$weather, &$forecast, $forecast_item, $forecast_items)
{
	...
	// DAYS OF WEEK
	$days_of_week = apply_filters('awesome_weather_days_of_week', array(__('Sun', 'awesome-weather-up'), __('Mon', 'awesome-weather-up'), __('Tue', 'awesome-weather-up'), __('Wed', 'awesome-weather-up'), __('Thu', 'awesome-weather-up'), __('Fri', 'awesome-weather-up'), __('Sat', 'awesome-weather-up')));
...
function awesome_weather_get_desc_from_id_openweathermaps($c)
{
	...
	// THUNDERSTORMS
	if ($c == 210) {
		return $codes['isolated-thunderstorms'];
	}
	if ($c == 212) {
		return $codes['severe-thunderstorms'];
	}
	if ($c >= 200 and $c < 300) {
		return $codes['thunderstorms'];
	}

	// DRIZZLE
	if ($c >= 300 and $c < 400) {
		return $codes['drizzle'];
	}

	// RAIN
	if ($c == 501) {
		return $codes['scattered-showers'];
	}
	if ($c == 511) {
		return $codes['freezing-rain'];
	}
	if ($c >= 500 and $c < 600) {
		return $codes['showers'];
	}

	// SNOW
	if ($c == 600) {
		return $codes['light-snow-showers'];
	}
	if ($c == 602) {
		return $codes['heavy-snow'];
	}
	if ($c == 611) {
		return $codes['sleet'];
	}
	if ($c == 621) {
		return $codes['snow-showers'];
	}
	if ($c >= 600 and $c < 700) {
		return $codes['snow'];
	}
	...
	// ATMOSPHERE
	if ($c == 701) {
		return __('mist', 'awesome-weather-up');
	}
	if ($c == 711) {
		return $codes['smoky'];
	}
	...
	if ($c == 751) {
		return __('sand', 'awesome-weather-up');
	}
	if ($c == 761) {
		return __('dust', 'awesome-weather-up');
	}
	if ($c == 762) {
		return __('volcanic ash', 'awesome-weather-up');
	}
	if ($c == 771) {
		return __('squalls', 'awesome-weather-up');
	}
	if ($c == 781) {
		return $codes['tornado'];
	}
	...
	if ($c == 801 or $c == 802 or $c == 803) {
		return $codes['partly-cloudy'];
	}
	if ($c == 804) {
		return $codes['mostly-cloudy'];
	}
	...
	if ($c == 901) {
		return $codes['tropical-storm'];
	}
	if ($c == 902) {
		return $codes['hurricane'];
	}
	if ($c == 903) {
		return $codes['cold'];
	}
	...
	if ($c == 952 or $c == 953 or $c == 954 or $c == 955 or $c == 956) {
		return $codes['breeze'];
	}
	if ($c == 957 or $c == 958 or $c == 959) {
		return $codes['windy'];
	}
	if ($c == 960) {
		return __('storm', 'awesome-weather-up');
	}
	if ($c == 961) {
		return __('violent storm', 'awesome-weather-up');
	}
	if ($c == 962) {
		return $codes['hurricane'];
	}
