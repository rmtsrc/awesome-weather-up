<?php
/*
Plugin Name: Awesome Weather Widget Updated
Plugin URI: https://halgatewood.com/awesome-weather-up
Description: A weather widget that actually looks cool
Author: Hal Gatewood Updated
Author URI: https://www.halgatewood.com
Version: 3.0.3
Text Domain: awesome-weather-up
Domain Path: /languages

Hi DEVS!
FILTERS AVAILABLE:
https://halgatewood.com/docs/plugins/awesome-weather-up-widget/available-filters
*/
...
$awesome_weather_sizes = apply_filters('awesome_weather_sizes', array(
	'tall' => __('Tall', 'awesome-weather-up'),
	'wide' => __('Wide', 'awesome-weather-up'),
	'micro' => __('Micro', 'awesome-weather-up'),
	'showcase' => __('Showcase', 'awesome-weather-up'),
	'long' => __('Long', 'awesome-weather-up'),
	'boxed' => __('Boxed', 'awesome-weather-up'),
	'material' => __('Material', 'awesome-weather-up'),
	'basic' => __('Basic', 'awesome-weather-up'),
	'custom' => __('Custom', 'awesome-weather-up')
));
...
include_once(dirname(__FILE__) . '/awesome-weather-up-codes.php');
include_once(dirname(__FILE__) . '/awesome-weather-up-mediabox.php');
...
{
	$locale = apply_filters('plugin_locale', get_locale(), 'awesome-weather-up');
	$mofile = WP_LANG_DIR . '/awesome-weather-up/awesome-weather-up-' . $locale . '.mo';

	if (file_exists($mofile)) {
		load_textdomain('awesome-weather-up', $mofile);
	} else {
		load_plugin_textdomain('awesome-weather-up', false, dirname(plugin_basename(__FILE__)) . '/languages/');
	}
}
...
{
	wp_enqueue_script('awesome_weather', plugin_dir_url(__FILE__) . 'js/awesome-weather-up-widget-frontend.js', array('jquery'), '1.1', add_filter('awesome_weather_js_in_footer', false));
	wp_enqueue_style('awesome-weather-up', plugins_url('/awesome-weather-up.css', __FILE__));
	...
	if ($use_google_font) {
		...
		wp_add_inline_style('awesome-weather-up', ".awesome-weather-up-wrap { font-family: 'Open Sans', sans-serif; font-weight: 400; font-size: 14px; line-height: 14px; }");
	}
}
...
// THE SHORTCODE
add_shortcode('awesome-weather-up', 'awesome_weather_shortcode');
add_shortcode('awesome-weather-up-ajax', 'awesome_weather_ajax');
...
function awesome_weather_logic($atts)
{
	...
	$weather->t->humidity 				= __('humidity', 'awesome-weather-up');
	$weather->t->high 					= __('H', 'awesome-weather-up');
	$weather->t->low 					= __('L', 'awesome-weather-up');
	$weather->t->now 					= __('Now', 'awesome-weather-up');
	$weather->t->wind 					= __('wind:', 'awesome-weather-up');
	$weather->t->weather_from 			= __('Weather from', 'awesome-weather-up');
	$weather->t->set_location 			= __('Set Your Location', 'awesome-weather-up');
	$weather->t->search_placeholder 	= __('Search: City, State or Country', 'awesome-weather-up');
	$weather->t->city_not_found 		= __('City not found, please try again.', 'awesome-weather-up');
	...
	if ($weather->background_color) {
		if (substr(trim($weather->background_color), 0, 1) != '#' and substr(trim(strtolower($weather->background_color)), 0, 3) != 'rgb' and $weather->background_color != 'transparent') {
			$weather->background_color = '#' . $weather->background_color;
		}
	...
	// BACKGROUND CLASSES
	$weather->background_classes[] = 'awesome-weather-up-wrap';
	...
	if ($weather->background_by_weather and ($weather_code or $weather_description_slug)) {
		...
		// CHECK FOR awesome-weather-up-bgs
		if (file_exists(dirname(__FILE__) . '/../awesome-weather-up-bgs/awe-backgrounds/')) {
			foreach ($bg_img_names as $bg_img_name) {
				if (file_exists(dirname(__FILE__) . '/../awesome-weather-up-bgs/awe-backgrounds/' . $bg_img_name)) {
					$weather->background_image = trailingslashit(plugins_url()) . 'awesome-weather-up-bgs/awe-backgrounds/' . $bg_img_name;
					break;
				}
			}
	...
	// PREP INLINE STYLE
	$inline_style = "";
	if ($weather->inline_style != '') {
		$inline_style = " style=\"{$weather->inline_style}\"";
	}
	...
	if ($weather->template == 'custom' or !isset($awesome_weather_sizes[$weather->template])) {
		...
		if ($user_defined_template_location) {
			...
			echo awesome_weather_error(__('Custom template file not found. Please add a file to your theme folder with this name:', 'awesome-weather-up') . ' <span style="text-transform: lowercase">awe-' . $weather->template . '.php</span>');
		}
	} else {
		...
		if (file_exists($awe_weather_template)) {
			...
			echo awesome_weather_error(__('Weather template not found:', 'awesome-weather-up') . ' ' . $weather->template);
		}
	}
...
function awesome_weather_error($msg = false)
{
	...
	if (!$msg) $msg = __('No weather information available', 'awesome-weather-up');

	if ($error_handling == 'display-admin') {
		// DISPLAY ADMIN
		if (current_user_can('manage_options')) {
			return "<div class='awesome-weather-up-error'>" . $msg . "</div>";
		}
	} else if ($error_handling == 'display-all') {
		// DISPLAY ALL
		return "<div class='awesome-weather-up-error'>" . $msg . "</div>";
	} else {
		return apply_filters('awesome_weather_error', "<!-- awesome-weather-up-error: " . $msg . " -->");
	}
}
...
function awesome_weather_admin_scripts($hook)
{
	...
	wp_enqueue_script('awesome_weather_admin_script', plugin_dir_url(__FILE__) . '/js/awesome-weather-up-widget-admin.js', array('jquery', 'underscore'));
	wp_enqueue_style('wp-color-picker');
	wp_enqueue_script('wp-color-picker');

	wp_localize_script(
		'awesome_weather_admin_script',
		'awe_script',
		array(
			'no_owm_city'				=> esc_attr(__('No city found in OpenWeatherMap.', 'awesome-weather-up')),
			'one_city_found'			=> esc_attr(__('Only one location found. The ID has been set automatically above.', 'awesome-weather-up')),
			'confirm_city'				=> esc_attr(__('Please confirm your city:', 'awesome-weather-up'))
		)
	);
}
...
function awe_widget_id(&$weather, $rtn = false)
{
	...
	if (!$weather->id and isset($weather->latlng) and $weather->latlng != '') $weather->id = 'awesome-weather-up-' . sanitize_title($weather->latlng);


	// USE LOCATION
	if (!$weather->id and isset($weather->location) and $weather->location != '') $weather->id = 'awesome-weather-up-' .  sanitize_title($weather->location);


	// USE owm_city_id
	if (!$weather->id and isset($weather->owm_city_id) and $weather->owm_city_id != '') $weather->id = 'awesome-weather-up-' . $weather->owm_city_id;


	// CREATE RANDOM
	if (!$weather->id) $weather->id = 'awesome-weather-up-' .  uniqid();
...
function awe_get_latlng_ajax()
{
	...
	if (!$location_iq_token and !$owm_appid) {
		echo json_encode(array('error' => __('No method of lookup available. Please enter an OpenWeatherMap APPID or LocationIQ Token in the Settings.', 'awesome-weather-up'), 'latlng' => ''));
		die;
	}
	...
	if (isset($_GET['location'])) {
		...
		if ($latlng) {
			...
			echo json_encode(array('error' => __('Location could not be geocoded.', 'awesome-weather-up'), 'latlng' => ''));
			die;
		}
	}
...
function awe_change_weather_form($weather)
{
	...
	$template = locate_template(array('awesome-weather-up-form.php'));
	if ($template) {
		include($template);
	} else {
		include(dirname(__FILE__) . '/awesome-weather-up-form.php');
	}
}
...
function awe_change_weather_trigger($weather)
{
	...
	$template = locate_template(array('awesome-weather-up-trigger.php'));
	if ($template) {
		include($template);
	} else {
		include(dirname(__FILE__) . '/awesome-weather-up-trigger.php');
	}
}
...
function awe_attribution($weather)
{
	...
	if ($weather->provider == 'darksky') {
		...
		$attr_text .= __('Powered by Dark Sky', 'awesome-weather-up');
		$attr_text .= '</a>';
	} else {
		$attr_text = $weather->t->weather_from . ' OpenWeatherMap';
	}

	echo apply_filters('awesome_weather_attribution', '<div class="awesome-weather-up-attribution">' . $attr_text . '</div>');
}
...
function awe_extended_link(&$weather, $do_before = '', $do_after = '')
{
	...
	if (!$weather->extended_text) $weather->extended_text = apply_filters('awesome_weather_extended_forecast_text', __('extended forecast', 'awesome-weather-up'));

	// ESCAPE THE WEATHER URL IF WE HAVE IT
	if ($weather->extended_url) $weather->extended_url = esc_url($weather->extended_url);


	$template = locate_template(array('awesome-weather-up-extended.php'));
	if ($template) {
		echo $do_before;
		include($template);
		echo $do_after;
		...
		echo '<div class="awesome-weather-up-more-weather-link">';
...
// SETTINGS
require_once(dirname(__FILE__) . '/awesome-weather-up-settings.php');
...
function awesome_weather_use_template($template_name, $weather)
{
	...
	if ($weather->template != 'custom') {
		...
		if ($user_defined_template_location) {
			...
			echo awesome_weather_error(__('Custom template file not found. Please add a file to your theme folder with this name:', 'awesome-weather-up') . " awe-" . $template_name . ".php");
		}
	}
}
