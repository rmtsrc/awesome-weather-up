<div id="<?php awe_widget_id($weather); ?>" class="<?php echo $background_classes ?>" <?php echo $inline_style; ?>>
	...
	<?php if ($weather->background_image) { ?>
		<div class="awesome-weather-up-cover" style="background-image: url(<?php echo $weather->background_image; ?>);">
			...
			<?php if (isset($weather->data['current'])) { ?>
				...
				<?php if ($weather->show_icons) { ?><span class="awesome-weather-up-stats-icon"><i class="<?php echo $weather->data['current']['icon']; ?>"></i></span><?php } ?>
				<span class="awesome-weather-up-current-temp"><?php echo $weather->data['current']['temp']; ?><sup>&deg;</sup></span><!-- /.awesome-weather-up-current-temp -->
			...
			<?php if ($weather->forecast_days != "hide") { ?>
				...
				<div class="awesome-weather-up-forecast-text awe_days_<?php echo count($weather_forecast); ?> awecf">

					<?php foreach ($weather_forecast as $forecast) { ?>
						<span class="awesome-weather-up-forecast-day-text">
							<?php if ($weather->show_icons) { ?><i class="<?php echo $forecast->icon; ?>"></i><?php } ?>
							<span class="awesome-weather-up-forecast-day-temp-text"><?php echo $forecast->high; ?><sup>&deg;</sup></span>
							<span class="awesome-weather-up-forecast-day-abbr-text"><?php echo $forecast->day_of_week; ?></span>
						</span>
					<?php } ?>
	...
	<?php } ?>
</div><!-- /.awesome-weather-up-wrap: basic -->